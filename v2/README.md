# TunerSwap Vendure Backend (v2)

This is the Vendure 2 backend for TunerSwap marketplace. It includes custom plugins for marketplace functionality, visitor analytics, and seller management.

## Custom Plugins

### MarketplacePlugin (`/src/plugins/marketplace/`)
Core marketplace functionality for the tuner/seller ecosystem.

**Key Features:**
- **SellerProfile Entity** - Stores seller business info, tuning expertise, social links, business hours
- **Channel-based Vendor Isolation** - Each seller gets dedicated Vendure Channel
- **Seller Registration** - Creates seller profiles with admin credentials
- **Admin Viewer System** - Admins can view any seller's dashboard

**Recent Updates (v1.4.0):**
- Added `youtube` field to SellerProfile entity
- All profile fields now persist correctly (hours, socials, tune types, experience, hasDyno)
- Business hours stored as JSON with new structured format

### VisitorAnalyticsPlugin (`/src/plugins/visitor-analytics/`)
...
- Customer IP history for security

**Recent Fixes (v1.4.0):**
- Fixed GraphQL schema mismatches causing dashboard errors
- `DashboardStats` now returns all required fields
- `DeviceBreakdown` returns proper array structure
- All time series and country data aligned with schema

---

## Original Vendure Documentation
