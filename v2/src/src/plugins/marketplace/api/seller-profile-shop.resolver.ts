...
import { Ctx, RequestContext, Allow, Permission, Transaction, TransactionalConnection, CustomerService, ProductService, ProductVariantService, OrderService, LanguageCode, TaxCategoryService, ChannelService, Zone } from '@vendure/core';
...
export class SellerProfileShopResolver {
    ...
    async mySellerOrders(
        ...
        // Query orders - for now return empty as we need proper order-seller association
        // In production, orders would have a sellerId field or be associated via products
        return { items: [], totalItems: 0 };
    }
    ...
    async createInvoiceListing(
        ...
        try {
            ...
            // If sendEmail is true, mark as sent (email sending would be handled by a service)
            if (args.input.sendEmail) {
                await this.connection.rawConnection
                    .getRepository('product')
                    .update(product.id, { customFieldsInvoicestatus: 'sent' });
                console.log(`[Invoice] Invoice marked as sent, would send email to ${args.input.recipientEmail}`);
            }
    ...
    async cancelInvoice(
        ...
        try {
            ...
            await this.connection.rawConnection
                .getRepository('product')
                .update(args.invoiceId, { customFieldsInvoicestatus: 'cancelled' });
    ...
    async resendInvoiceEmail(
        ...
        try {
            ...
            await this.connection.rawConnection
                .getRepository('product')
                .update(args.invoiceId, { customFieldsInvoicestatus: 'sent' });
    ...
    async invoiceByToken(
        ...
        try {
            ...
            const variantResult = await this.productVariantService.getVariantsByProductId(ctx, (rawProduct as any).id);
            const defaultVariant = variantResult.items[0];

            const translation = (rawProduct as any).translations?.[0];
            const sellerId = (rawProduct as any).customFieldsSellerid;
            ...
            return {
                id: (rawProduct as any).id.toString(),
                productId: (rawProduct as any).id.toString(),
                token: (rawProduct as any).customFieldsInvoicetoken,
                name: translation?.name || 'Invoice',
                description: translation?.description || '',
                price: defaultVariant?.price || 0,
                recipientCustomerId: (rawProduct as any).customFieldsRecipientcustomerid,
                recipientEmail: (rawProduct as any).customFieldsRecipientemail,
                notes: (rawProduct as any).customFieldsInvoicenotes,
                lineItems: (rawProduct as any).customFieldsInvoicelineitems,
                status: (rawProduct as any).customFieldsInvoicestatus || 'draft',
                createdAt: (rawProduct as any).createdAt,
                expiresAt: (rawProduct as any).customFieldsInvoiceexpiresat,
                paidAt: (rawProduct as any).customFieldsInvoicepaidat,
                seller,
