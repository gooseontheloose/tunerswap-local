...
// Initialize Stripe (lazy initialization to avoid crash when key not set)
...
const SERVER_SECRET = process.env.VENDOR_SERVER_SECRET || process.env.COOKIE_SECRET || 'server-to-server-secret';
...
export const resolvers = {
  ...
  Query: {
    ...
    myPayoutInfo: async (_: unknown, __: unknown, ctx: VendorContext) => {
      requireAuth(ctx);

      // Get pending orders amount
      const pendingOrders = await ctx.prisma.order.aggregate({
        where: {
          vendorId: ctx.vendorId,
      ...
      // Get last payout
      const lastPayout = await ctx.prisma.payout.findFirst({
        where: { vendorId: ctx.vendorId, status: 'COMPLETED' },
        orderBy: { createdAt: 'desc' },
      });

      // Get payout mode from platform settings
      const payoutModeSetting = await ctx.prisma.platformSettings.findUnique({
        where: { key: 'payout_mode' },
      });

      return {
        pendingAmount: pendingOrders._sum.vendorPayout || 0,
        lastPayoutDate: lastPayout?.processedAt || null,
        lastPayoutAmount: lastPayout?.amount || null,
        payoutMode: payoutModeSetting?.value || 'instant',
      };
    },

    myPayoutHistory: async (
      _: unknown,
      ...
      const [items, totalItems] = await Promise.all([
        ...
        ctx.prisma.payout.count({ where: { vendorId: ctx.vendorId } }),
      ]);
  ...
  Mutation: {
    ...
    /**
     * Server-to-server authentication for Vendure integration
     * Allows the website server to get a vendor JWT for authenticated sellers
     * without requiring the seller's password.
     *
     * Auto-creates seller in MarketplaceDB if they don't exist (for backwards compatibility
     * with existing Vendure sellers who haven't been synced yet).
     */
    authenticateFromVendure: async (
      _: unknown,
      args: { email: string; serverSecret: string; firstName?: string; lastName?: string },
      ctx: VendorContext
    ) => {
      // Verify server secret
      if (args.serverSecret !== SERVER_SECRET) {
        console.log('[Auth] Invalid server secret attempt');
        throw new GraphQLError('Invalid server secret', {
          extensions: { code: 'UNAUTHENTICATED' },
        });
      }
      ...
      let seller = await ctx.prisma.seller.findUnique({
        where: { email },
      });

      // Auto-create seller if they don't exist (backwards compatibility)
      if (!seller) {
        console.log('[Auth] Auto-creating seller for:', email);

        // Generate unique slug from email
        const baseSlug = email.split('@')[0].replace(/[^a-z0-9]/gi, '-').toLowerCase();
        let slug = baseSlug;
        let counter = 1;
        while (await ctx.prisma.seller.findUnique({ where: { slug } })) {
          slug = `${baseSlug}-${counter}`;
          counter++;
        }

        try {
          seller = await ctx.prisma.seller.create({
            data: {
              email,
              slug,
              firstName: args.firstName || email.split('@')[0],
              lastName: args.lastName || '',
              passwordHash: '', // No password - uses Vendure auth
              status: 'APPROVED',
              verified: false,
              rating: 0,
              reviewCount: 0,
          ...
          console.log('[Auth] Created seller:', seller.id, seller.email);
        } catch (error) {
          console.error('[Auth] Failed to create seller:', error);
          return null;
        }
      }
    ...
    initiateStripeOnboarding: async (
      ...
      try {
        ...
        let accountId = ctx.vendor.stripeAccountId;
        ...
        if (!accountId) {
          ...
          const account = await getStripe().accounts.create({
            ...
            business_profile: {
              name: ctx.vendor.businessName || `${ctx.vendor.firstName} ${ctx.vendor.lastName}`,
            },
            metadata: {
              vendor_id: ctx.vendorId.toString(),
              platform: 'tunerswap',
            },
          });
        ...
        const accountLink = await getStripe().accountLinks.create({
          account: accountId,
          refresh_url: `${WEBSITE_URL}/selleradmin/profile?stripe_refresh=true`,
          return_url: `${WEBSITE_URL}/selleradmin/profile?stripe_success=true`,
        ...
        return {
          success: false,
          error: error instanceof Error ? error.message : 'Failed to initiate Stripe onboarding',
        };
      }
    },
    ...
    getStripeDashboardLink: async (
      ...
      try {
        const loginLink = await getStripe().accounts.createLoginLink(ctx.vendor.stripeAccountId);

        return {
          url: loginLink.url,
          expiresAt: new Date(Date.now() + 60 * 1000), // Link expires in ~1 minute
        };
      } catch (error) {
        throw new GraphQLError('Failed to generate dashboard link', {
          extensions: { code: 'INTERNAL_SERVER_ERROR' },
        });
      }
    },

    refreshStripeAccountStatus: async (
      _: unknown,
      __: unknown,
      ctx: VendorContext
    ) => {
      ...
      try {
        const account = await getStripe().accounts.retrieve(ctx.vendor.stripeAccountId);
        ...
        throw new GraphQLError('Failed to refresh Stripe account status', {
          extensions: { code: 'INTERNAL_SERVER_ERROR' },
        });
      }
    },
    ...
    disconnectStripe: async (
      ...
      // Note: This doesn't delete the Stripe account, just disconnects it from the platform
      const seller = await ctx.prisma.seller.update({
        where: { id: ctx.vendorId },
        data: {
          stripeConnected: false,
  ...
  Order: {
    lines: async (parent: { id: number; lines?: any[] }, ctx: VendorContext) => {
      // If lines are already loaded (via include), return them directly
      if (parent.lines) {
        return parent.lines;
      }
